document.addEventListener('DOMContentLoaded', function () {
		...
		*/
		...
		const emElement = document.querySelector('em:first-of-type');
		// STEP 3: Assign a click event listener to the above element, and include a reference to the function to be invoked
		emElement.addEventListener('click', getUserInput);
			// STEP 3a: Create a function that waits for the above click event, and include a parameter to receive the event object
		function getUserInput(event) {
			...
			if (userInput) {
				...
				emElement.textContent = 'teacher Scott';
			}
		}
		// emElement.addEventListener('click', function () {}); //alternative means of adding event listener function, but not as good for readability and reusability
