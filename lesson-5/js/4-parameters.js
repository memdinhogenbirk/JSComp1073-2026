...
async function startProgram() {
    ...
    await speak(`My current position is X: ${Math.trunc(currentPosition.x)}, Y: ${Math.trunc(currentPosition.y)}`);
...
async function traceSquare(distance, velocity){
    ...
    await speak(`${distance} centimeters squared`);
